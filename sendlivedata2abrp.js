...
function DisplayLiveData(myJSON,CR) {
    ...
    newcontent += "elevation=" + myJSON.altitude + CR;    //GPS altitude
...
function GetUrlABRP(myJSON,CR) {
    ...
    urljson += "elevation=" + myJSON.altitude + CR;    //GPS altitude
...
exports.send = function(onoff) {
    if (onoff) {
        ...
        objTimer = PubSub.subscribe("ticker.30", SendLiveData); // update each 60s
    }
    else {
