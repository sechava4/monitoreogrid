...
features["slope_cat"] = pd.cut(features["slope"], np.arange(-8.5, 8.6, 3.4)).astype(
    "string"
)

# Load the mean power feature for all users
mean_max_pot_by_slope = pd.read_csv(
    path + "/Develops/ConsumptionEstimationJournal/mean_max_features_by_slope.csv",
    index_col=0,
)
...
def test(p, m):
    ...
    r_forest_reg = load(
        open(
            path
            + "/Develops/ConsumptionEstimationJournal/randomForest_0_13_mean_consumption_maxerr_model.pkl",
            "rb",
        )
    )

    # load ANN regressor
    # ann_reg = load(open(path + '/Develops/ConsumptionEstimationJournal/ann_regr.pkl', 'rb'))
