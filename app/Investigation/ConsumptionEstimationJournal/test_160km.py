...
mean_features_by_slope = pd.read_csv(
    path + "/Develops/ConsumptionEstimationJournal/mean_features_by_slope.csv"
)
...
def test(p, m):
    ...
    scaler = load(
        open(path + "/Develops/ConsumptionEstimationJournal/scaler_lm.pkl", "rb")
    )
    ...
    # Load linear model
    lm_cons = sm.load(path + "/Develops/ConsumptionEstimationJournal/lm_consumo.pickle")
    lm_cons = load(
        open(path + "/Develops/ConsumptionEstimationJournal/linear_model.pkl", "rb")
    )
    ...
    r_forest_reg = load(
        open(
            path
            + "/Develops/ConsumptionEstimationJournal/randomForest_0_16_mean_consumption_maxerr_model.pkl",
            "rb",
        )
    )
    ...
    xgb_reg = load(
        open(
            path + "/Develops/ConsumptionEstimationJournal/xg_reg_model.pickle.dat",
            "rb",
        )
    )
    ...
    # load ANN regressor
    ann_reg = load(
        open(path + "/Develops/ConsumptionEstimationJournal/ann_regr.pkl", "rb")
    )
    ...
    # Load inverse scaler
    scaler_inv = load(
        open(path + "/Develops/ConsumptionEstimationJournal/scaler.pkl", "rb")
    )
