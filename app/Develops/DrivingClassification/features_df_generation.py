...
def feature_extraction(trace):
    ...
    num_acc_minuto, num_acc_fr_minuto, prom_sobrepaso_acc, prom_sobrepaso_fren, prom_abs_acc, std_acc, max_acc = peak_features(
    ...
    num_jerk_acc_minuto, num_jerk_freno_minuto, prom_sobrepaso_jerk_acc, prom_sobrepaso_jerk_freno, prom_abs_jerk, std_jerk, max_jerk = peak_features(
        trace, jerk[1:], 1.5, 1.5, 'jerk')  # mas de nedio acelerador

    # Picos corriente
    current = trace['current'].to_numpy()
    num_current_minuto, num_current_fr_minuto, prom_sobrepaso_current, prom_sobrepaso_current_fr, prom_abs_current, std_current, max_current = peak_features(
        trace, current, 115, 100, 'current')  # mas de nedio acelerador

    std_potencia = np.std(trace['power_kw'])
    prom_abs_potencia = np.mean(np.absolute(trace['power_kw'] ))
    ...
    return [num_acc_minuto, num_acc_fr_minuto, prom_sobrepaso_acc, prom_sobrepaso_fren, prom_abs_acc, std_acc,
            num_jerk_acc_minuto, num_jerk_freno_minuto, prom_sobrepaso_jerk_acc,
            prom_sobrepaso_jerk_freno, prom_abs_jerk, std_jerk, std_potencia, prom_abs_potencia,  consumption,
            kms, consumption_per_km, num_current_minuto, num_current_fr_minuto, prom_sobrepaso_current,
            ...
            trace['power_kw'].max(), max_speed, mean_speed, std_speed]
...
if __name__ == '__main__':
    ...
    cols = ['num_acc_minuto', 'num_acc_fr_minuto', 'prom_sobrepaso_acc', 'prom_sobrepaso_fren', 'prom_abs_acc',
            ...
            'std_current_std_jerk', 'highway', 'slope', 'max_current', 'max_jerk', 'max_acc', 'max_power',
    ...
    sns.pairplot(features.dropna(), hue='highway', vars=['max_current', 'std_potencia', 'slope', 'prom_abs_current',
                                                         'max_power', 'max_jerk', 'consumption',
                                                         'max_speed'], kind='scatter')
    '''

    le = LabelEncoder()
    features['highway_enc'] = le.fit_transform(features['highway'])
    X = np.array(features[['max_current', 'std_potencia', 'slope', 'prom_abs_current',
                           'max_power', 'max_jerk', 'consumption_per_km',
