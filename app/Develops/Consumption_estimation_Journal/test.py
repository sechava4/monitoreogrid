...
def gen_test_traces(df):
    ...
    for index, row in df.iterrows():
        ...
        if suma >= 1200 or row['operative_state'] >= 3 or (old_name != row['name'] and not nan):  # pendiente
            suma = 0
            trace_id += 1
            aux_trace_id -= 1
...
measure_segments['slope_cat'] = pd.cut(measure_segments["slope"], np.arange(-8.5, 8.6, 3.4)).astype('string')
...
mean_max_pot_by_slope = pd.read_csv(path + '/Develops/Consumption_estimation_Journal/mean_max_features_by_slope.csv',
                                    index_col=0)
...
def test(i):
    ...
    m = pd.merge(left=m, right=mean_max_pot_by_slope, left_on='slope_cat', right_on='slope_cat')
    ...
    m = pd.merge(left=m, right=mean_max_p_per_user_and_slope,
    ...
    p = pd.merge(left=p, right=mean_max_pot_by_slope,
                 left_on=['slope_cat'], right_on=['slope_cat'])

    aux_p = pd.merge(left=p, right=mean_max_p_per_user_and_slope,
    ...
    r_forest_reg = load(open(path + '/Develops/Consumption_estimation_Journal/randForest_model.pkl', 'rb'))
    ...
    p_scaled['consumption_per_km'] = ann_reg.predict(p_scaled[['mean_max_power_usr', 'mean_soc',
    ...
    fig = go.Figure([go.Scatter(x=p['end_time'], y=p['consumption_pred'], name='Predicted')])
    ...
    fig2 = go.Figure([go.Scatter(x=p['end_time'], y=p['consumption_pred'].cumsum(), name='Predicted')])
    fig2.add_trace(go.Scatter(x=p['end_time'], y=p['consumption'].cumsum(), name='Measured'))
    fig2.update_layout(
        title="Model evaluation for route " + str(i),
        xaxis_title="Time",
        yaxis_title="kWh/km")
    ...
    time.sleep(2)
    error = 100 * (p['consumption'].cumsum().iloc[-1] - p['consumption_pred'].cumsum().iloc[-1]) / p['consumption'].cumsum().iloc[-1]
...
for t in range(5, 14):  # [5, 7, 10, 13, 15]:
    ...
    plot_consolidate = df_to_consolidate[df_to_consolidate['test_id'] == t].reset_index(drop=True)
    plot_test = df_to_test[df_to_test['test_id'] == -t].reset_index(drop=True)
    if (len(plot_consolidate) > 1) and (len(plot_test) > 1):
        print('measure group', t, 'length=',
              plot_consolidate['odometer'].iloc[-1] - plot_consolidate['odometer'].iloc[0])
        print('test group no:', t, 'lenght=', plot_test['odometer'].iloc[-1] - plot_test['odometer'].iloc[0])
