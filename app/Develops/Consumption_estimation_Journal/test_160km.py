...
features.loc[features['cumdist'].between(25, 105), 'user_id'] = 'Santiago_Echavarria_test1'

features.loc[features['cumdist'].between(105, 130), 'user_id'] = 'Santiago_Echavarria_measure2'
...
mean_features_by_slope = pd.read_csv(path + '/Develops/Consumption_estimation_Journal/mean_features_by_slope.csv')
...
def test(p, m):
    ...
    p = pd.merge(how='left', left=p, right=mean_features_by_user_and_slope,
                 left_on=['user_id', 'slope_cat'], right_on=['user_id', 'slope_cat'])

    # -------------------------- Add mean for all users atributes to prediction dataset--------------------------------------------
    p = pd.merge(how='left', left=p, right=mean_features_by_slope,
...
test_test1 = test_test1[(test_test1.consumption < 0.6) & (test_test1.consumption > -0.3)]
...
test_test2 = test_test2[(test_test2.consumption < 0.6) & (test_test2.consumption > -0.3)]
